(function ( d ) {
	...
	cnv.on('selection:cleared', _cancelEdit); // Object deselecting event handler
	...
	function _chooseDevice ( /*[Object] event*/ e ) {
		...
		removeObjects(window.$textObjects);
		removeObjects(window.$imageObjects);
		removeObjects(window.$textObjects);
		removeObjects(window.$imageObjects);
		renderSpine();
	...
	function setOverlay() {
		for (var i = 0; i < overlay.objectTitles.length; i++) {
			...
			overlay[overlay.objectTitles[i]].object.set({
				fill: overlay[overlay.objectTitles[i]].fill,
	...
	function setObjectsOrder () {
		console.log( 'cnv._objects', cnv._objects );
	...
	function removeObjects ( /*[Canvas object] array*/ _Objects ) {
		for (var i = 0; i <= _Objects.length; i++) {
			cnv.remove(_Objects[i]);
			if( is_text(_Objects[i]) ) {
				closeEditTextBar();
				cnv.remove(_Objects[i]);
				window.$textObjects.splice(window.$textObjects.indexOf(_Objects[i]), 1);
			}
			else if ( is_image(_Objects[i]) ) {
				window.$imageObjects.splice(window.$imageObjects.indexOf(_Objects[i]), 1);
			}
		}
	}
