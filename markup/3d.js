(function ( d ) {
	...
	function removeObjects ( /*[Canvas object] array*/ Objects ) {
		for (var i = 0; i <= Objects.length; i++) {
			cnv.remove(Objects[i]);
			if( is_text(Objects[i]) ) {
				closeEditTextBar();
				cnv.remove(Objects[i]);
				window.$textObjects.splice(window.$textObjects.indexOf(Objects[i]), 1);
			}
			else if ( is_image(Objects[i]) ) {
				window.$imageObjects.splice(window.$imageObjects.indexOf(Objects[i]), 1);
			}
		}
	}
